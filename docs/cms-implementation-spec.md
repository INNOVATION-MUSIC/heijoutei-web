...
- **店舗宛のテイクアウト注文通知（`buildStoreMail`）には個人情報を載せない**。受取店舗・受取日時・注文明細・合計・注文番号＋「管理画面で詳細を見る」リンク（`${SITE_URL}/admin/order/<id>`）のみ。氏名・連絡先はリンク先で確認する（メール流出・宛先タイポによる第三者流出の防止）。
- リンク先 `/admin/order/[id]` は **サイドバー等を出さずその注文だけを表示する単独ページ**（`(protected)` 外・ページ自身で `getUser()` ＋ 未ログインは `LoginCard` インライン表示＋店舗スコープ `notFound()`）。他の注文は見えない。
