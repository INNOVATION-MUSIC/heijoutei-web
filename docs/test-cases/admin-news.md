...
## ダッシュボード「最近のお知らせ」（DASH・2026-06-11）

対象: `/admin`（`src/app/admin/(protected)/page.tsx`）の「最近のお知らせ」ブロック。

設計メモ:
- 各行＝**公開日付（JST `YYYY/MM/DD`・下書きは「—」）→ 公開/下書きバッジ → タグ（色ピル）→ タイトル** の順。
- 行全体が `/admin/news/[id]/edit` への Link（ホバー反応）。
- **タグ列は固定幅 `w-28` ＋ `overflow-hidden`** で、タグ数が増えても見切れるだけ・**タイトル左端は全行で整列**（3 個目以降のタグは見切れる＝整列優先・ユーザー合意）。

| ID | テスト項目 | 前提条件 | 操作手順 | 期待結果 | 優先度 |
|----|-----------|----------|----------|----------|--------|
| DASH-001 | 最近のお知らせ表示 | ログイン・news 複数件 | `/admin` を開く | 公開日付→公開/下書きバッジ→タグ→タイトルの順で行が並ぶ | 中 |
| DASH-002 | 下書きの日付表示 | 下書き記事あり | `/admin` を開く | 下書き行の公開日付が「—」で表示される | 低 |
| DASH-003 | 行クリックで編集へ | お知らせあり | 行をクリック | `/admin/news/[id]/edit` に遷移する | 中 |
| DASH-004 | タグ多数でもタイトル整列 | タグ 0〜8 個の記事が混在 | `/admin` を開く | タグ列は固定幅で見切れるが、全行のタイトル左端が揃う | 低 |
