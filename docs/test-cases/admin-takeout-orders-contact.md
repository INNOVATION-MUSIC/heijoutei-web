...
| ORD-013 | 店舗通知メールに個人情報が載らない | Brevo設定済 | テイクアウト注文を送信し、店舗宛メール（`buildStoreMail`）を確認 | 件名・本文に**氏名/フリガナ/メール/電話/連絡事項が無い**。受取店舗・受取日時・注文明細・合計・注文番号（`#`＋id先頭8桁）・「管理画面で詳細を見る」リンク（`${SITE_URL}/admin/takeout-orders?order=<id>`）がある。お客様控え（`buildCustomerMail`）は従来どおり氏名・明細入り | 高 |
| ORD-014 | メールのリンクで対象注文が開く | 注文あり・ログイン済 | 店舗メールの「管理画面で詳細を見る」を開く（`?order=<id>`） | その注文カードが自動で `open` 展開＋ゴールドのリングで一時強調（`OrderDeepLink`）。未読/既読フィルタは無視される。対象が一覧に無い（店舗スコープ外・削除済み）場合は素通り | 高 |
| ORD-015 | 未ログインでメールのリンクを開く | ログアウト状態 | `/admin/takeout-orders?order=<id>` を直接開く | `/admin/login` へリダイレクトされず、**URL が保たれたままインラインでログインフォーム表示**。ログイン成功で同じ URL のまま対象注文が展開される | 高 |
