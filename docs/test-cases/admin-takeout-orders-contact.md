...
| ORD-013 | 店舗通知メールに個人情報が載らない | Brevo設定済 | テイクアウト注文を送信し、店舗宛メール（`buildStoreMail`）を確認 | 件名・本文に**氏名/フリガナ/メール/電話/連絡事項が無い**。受取店舗・受取日時・注文明細・合計・注文番号（`#`＋id先頭8桁）・「管理画面で詳細を見る」リンク（`${SITE_URL}/admin/order/<id>`）がある。お客様控え（`buildCustomerMail`）は従来どおり氏名・明細入り | 高 |
| ORD-014 | メールのリンクは「その注文のみ」表示 | 注文あり・ログイン済 | 店舗メールの「管理画面で詳細を見る」（`/admin/order/<id>`）を開く | **サイドバー・トップバーなし**の単独ページに、その注文の詳細（店舗/日時/明細/合計/お客様情報）＋既読トグル＋ステータス変更＋「注文一覧をすべて見る」リンクのみ。他の注文は一切表示されない | 高 |
| ORD-015 | 未ログインでメールのリンクを開く | ログアウト状態 | `/admin/order/<id>` を直接開く | `/admin/login` へリダイレクトされず、**URL が保たれたままインラインでログインフォーム表示**。ログイン成功で同じ URL のままその注文詳細が表示される | 高 |
| ORD-016 | 担当外店舗の注文詳細 | 店舗スタッフ（例: 園部）でログイン | 他店（亀岡）の `/admin/order/<id>` を開く | 404（`notFound()`）。注文内容は表示されない | 高 |
