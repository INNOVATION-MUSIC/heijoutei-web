...
function pickNotifyEmails(raw: unknown, key: "takeout_notify_emails" | "contact_notify_emails"): string[] {
  const row = Array.isArray(raw) ? raw[0] : raw;
  const list = (row as Record<string, unknown> | null)?.[key];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      const notifyTo = store.notifyEmails.length > 0
        ? store.notifyEmails
        : (process.env.ORDER_NOTIFY_TO ? [process.env.ORDER_NOTIFY_TO] : []);
