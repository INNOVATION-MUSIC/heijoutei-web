...
export async function createNews(p: NewsPayload, tags: NewsTagInput[]) {
  ...
  const base = { title: p.title.trim(), body: p.body || null, thumbnail_url: p.thumbnail_url || null, hide_detail_thumbnail: !!p.hide_detail_thumbnail, thumbnail_contain: !!p.thumbnail_contain, is_published, published_at }
...
export async function updateNews(id: string, p: NewsPayload, tags: NewsTagInput[]) {
  ...
  const { error } = await adminSupabase
    .from('news')
    .update({ title: p.title.trim(), slug, body: p.body || null, thumbnail_url: p.thumbnail_url || null, hide_detail_thumbnail: !!p.hide_detail_thumbnail, thumbnail_contain: !!p.thumbnail_contain, is_published, published_at })
    .eq('id', id)
